...
void TNetwork::SendFileToClient(TClient& c, size_t Size, const std::string& Name) {
    ...
    std::string FailureReason;
    {
        ...
        const int socket = c.GetTCPSock().native_handle();
        size_t TotalSent = 0;
        while (TotalSent < Size) {
            off_t SysOffset = off_t(TotalSent);
            const ssize_t SentNow = sendfile(socket, fd, &SysOffset, Size - TotalSent);
            if (SentNow > 0) {
            ...
            if (SentNow == 0) {
                ...
                FailureReason = "sendfile returned 0 during mod download";
                break;
            }
            if (errno == EINTR) {
                continue;
            ...
            FailureReason = "sendfile failed during mod download";
            break;
        }
    }
    ::close(fd);
    if (!FailureReason.empty()) {
        DisconnectClient(c, FailureReason);
    }
